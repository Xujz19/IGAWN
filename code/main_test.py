...
parser.add_argument('--dataset_name', default='LOL_V2', type=str)
...
if __name__ == '__main__':
    ...
    low_path = os.path.join(args.test_data, 'Low')
    high_path = os.path.join(args.test_data, 'Normal')
    low_illumination_path = os.path.join(args.test_data, 'Normal_illumination_by_RetinexNet')
    ...
    data_time = time.time()
    image_names = []
    for root, _, fnames in sorted(os.walk(high_path)):
        for fname in fnames:
        ...
        if cuda:
            ...
            with torch.no_grad():
                for img_name in image_names:
                    ...
                    high_image = imageio.imread(os.path.join(high_path, high_name), pilmode='RGB')
                    low_image = imageio.imread(os.path.join(low_path, low_name))
                    low_image = util.uint2single(low_image)
                    low_illumination = imageio.imread(os.path.join(low_illumination_path, high_name))
                    low_illumination = low_illumination[:, :, 0:1]
                    ...
                    intensity_psnr = util.calculate_psnr(pred_img, high_image)
